namespace MieTranslationLib.Data.Chatter
{
    ...
    public class MieChatterNodeEntry
    {
        ...
        /// チャッターノードの辞書(ツリー形式)。
        /// キーは NodeID。
        /// チャッターツリーに存在せず、言語情報に存在する項目はルートノードとして登録する(ゾンビノード)
        /// </summary>
        private IDictionary<int, MieChatterNodeEntry> Nodes { get; } =
            new SortedDictionary<int, MieChatterNodeEntry>();
        ...
        public void GetSortedFlatNodes(IList<MieChatterNodeEntry> list, bool viewStopNode)
        {
            //// Stop node を表示する。
            //// 階層を辿りながら処理する。
            foreach (var node in this.Nodes.Values)
        ...
        public string ToLinkString(bool viewStopNode, int tabCount)
        {
            ...
            {
                if (this.Attribute.HasFlag(MieChatterNodeEntry.NNodeAttribute.StopNode))
                {
                    //// StopNodeは表示しない。
                    return string.Empty;
                }
            ...
            /**********************************************************************************/
            {
                ...
                if (this.Attribute.HasFlag(NNodeAttribute.StopNode))
                {
                    attributeText = "StopNode,";
                }

                if (this.Attribute.HasFlag(NNodeAttribute.MissingNode))
                {
                    attributeText += "MissingNode,";
                }

                if (this.Attribute.HasFlag(NNodeAttribute.None))
                {
                    attributeText += "None,";
                }
